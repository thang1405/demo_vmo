...
import { TABLE_NAME } from "../modules/project-status/project-status.constants";

const MultiSelectProjectStatus = ({ valueData, callBackData }) => {
  ...
  return (
    <div className="w-full relative py-0.5 my-1" ref={wrapperRef}>
      ...
      {show ? (
        <ul className="rounded-b scroll_hidden absolute overflow-scroll overflow-x-hidden max-h-32  z-10 bg-white w-full max-w-xs border border-indigo-400">
          {data.map((item, index) => (
            <li
...
export default MultiSelectProjectStatus;
