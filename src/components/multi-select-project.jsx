...
import { TABLE_NAME } from "../modules/projects/projects.constants";

const MultiSelectProject = ({ valueData, callBackData }) => {
  ...
  return (
    <div className="w-full relative py-0.5 my-1" ref={wrapperRef}>
      ...
      {show ? (
        <ul className=" rounded-b scroll_hidden absolute overflow-scroll overflow-x-hidden max-h-32 z-10 w-full max-w-xs bg-white border-l border-r border-b border-indigo-400">
          {data.map((item, index) => (
            <li
              className={`p-1 pl-3 border-b border-gray-300 ${
